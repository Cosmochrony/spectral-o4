...
\begin{document}
  ...
  \begin{abstract}
    ...
    $\sqrt{p(n)}$ factor in its concluding asymptotic step; none of these defects depends on
    the others.
    ...
    Interpretively, a conditional analysis identifies a further qualitative tension: if a
    fixed positive fraction of the available activation budget is realised and target
    congestion remains uniformly bounded, strong expansion favours fast, near-geometric
    exploration of the underlying group, the qualitative opposite of the slow polynomial
    growth a structural exponent bound requires.
    Neither condition is derived here.
  ...
  \section{Introduction}
    ...
    sub-programme as delivering a structural bound $\beta \leq 1$ (in the rescaled convention
    of Remark~\ref{rem:beta-convention}), independent of phenomenological calibration.
  ...
  \subsection{Setup and closure hypothesis}
    ...
    \begin{remark}[Notation convention: $\beta \leq 1$ as shorthand]
      ...
      Throughout this paper and the O-series, we adopt the following rescaling convention for
      $\beta$, used wherever a quadratic-type bound $p(n) \lesssim Cn^2$ is discussed.
      The saturated case $p(n) \sim C n^2$ is identified with the \emph{maximal admissible
      growth rate under such a bound}, and is assigned the rescaled exponent $\tilde\beta = 1$
      after the substitution $\tilde\beta := \beta/2$.
      Under this rescaling, a quadratic bound would read $\tilde\beta \leq 1$, and the
      ...
      denotes $\tilde\beta$ (i.e.\ the rescaled exponent), for consistency with the rest of
      the O-series.
      This convention is purely notational and does not by itself assert that any bound of
      this form holds; Section~\ref{ssec:defects} shows that the specific argument examined
      here does not establish one.
    \end{remark}
  ...
  \subsection{Three independent defects of the front-size argument}
    ...
    \begin{enumerate}
      ...
      \item[\textbf{Defect 3 (asymptotic error).}]
        Even granting the unsupported upper bound of Defect 2, combining it with the total
        outgoing flux bound $(p(n)+1)\,c_\chi$ and the Cheeger estimate
        $h(\mathcal{G}(n)) \gtrsim 1-1/\sqrt{p(n)}$ yields an estimate of order
  ...
  \subsection{What a repair would additionally require}
    ...
    \begin{enumerate}
      \item a persistent cross-rank exploration model compatible with the finite-window LPS
        host and the activation dynamics below; simple and multiplicity-weighted running
        unions of successive generating sets can be written down, but neither currently
        supplies every property the argument needs;
      \item a resolution of whether that host should be a weighted multigraph (with edge
        multiplicities requiring their own physical interpretation as parallel relational
        channels) or a simple graph (requiring a proved bound on cross-generator overlap, not
        currently available);
  ...
  \subsection{Interpretation: expansion and the direction of the missing argument}
    ...
    Vertex-isoperimetric inequalities convert a spectral gap into a \emph{lower} bound on
    available boundary size.
    A multiplicity-weighted running-union model of the exploration host admits an explicit
    strong finite-window spectral estimate; transferring it to a simple-union model requires
    an overlap bound not proved here.
    Even where such expansion control is available, geometric growth of the explored region
    follows only under two further assumptions: each inner-boundary vertex realises a fixed
    positive fraction of its activation budget, and activated edges have uniformly bounded
    congestion on exterior target vertices.
    Under those conditions the explored region reaches finite-group saturation on a
    logarithmic depth scale, rather than following the polynomial growth
    $p(n) \sim n^\beta$ that a structural exponent bound presupposes.
    This observation is explicitly \textbf{not} a theorem: neither activation utilisation nor
    bounded congestion is derived, and failure of either can avoid the conclusion.
    It identifies the burden of proof for a future repair: the mechanism must explain
    quantitatively why realised, distinct activation remains slow, rather than assume slow
    growth to recover the phenomenological target.
  ...
  \subsection{Scope of the open problem}
    ...
    i.e.\ to at most quadratic cumulative exploration, written $\beta \leq 1$ in the rescaled
    convention of Remark~\ref{rem:beta-convention}.
    ...
    the narrow phenomenological window $\beta^* \in (0.09, 0.13)$.
    \emph{If} a structural upper bound $\beta \leq 1$ were established by a repaired argument,
    it would be compatible with this window without explaining it: the gap between the two
    would be a factor of roughly $8$--$10$ in the exponent, itself requiring further structure
    to close.
  ...
  \subsection{Three sources of additional structure}
    ...
    \paragraph{Saturated Born--Infeld front dynamics.}
      The front-size argument examined in Section~\ref{ssec:defects} uses only the linear
      regime of the flux bound~\eqref{eq:bi-constraint}.
      In the fully saturated regime $|\partial_t \chi_v| \simeq c_\chi$, the nonlinear DBI
      dynamics introduces corrections that could further suppress front expansion.
      A refined analysis of the saturated DBI front, extending the one-mode reduction
      of~\cite{Beau2026a1} to the variable-valence setting, could yield a bound of the form
      $\Delta N(n) \lesssim c_\chi^\alpha \sqrt{p(n)}$ with $\alpha < 1$ -- but only once a
      correctly typed front-size estimate exists to refine in the first place.
  ...
  \subsection{Directions for further work}
    ...
    \paragraph{Effective dimension of $\Omega_\chi$.}
      ...
      the front bound -- once a correctly typed front bound exists to modify.
      Determining $D$ from the spectral structure of the Cosmochrony substrate is amenable to
      the techniques of~\cite{Beau2026a}.

    \paragraph{Saturated Born--Infeld front dynamics.}
      The front-size argument of Section~\ref{ssec:defects} uses only the linearised regime
      of the flux constraint.
      In the fully saturated regime $|\partial_t \chi_v| \simeq c_\chi$, the nonlinear DBI
      dynamics could suppress front expansion more strongly.
      ...
      but again only once a correctly typed estimate exists to refine.

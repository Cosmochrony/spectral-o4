...
\hypersetup{
  ...
  pdfkeywords={relational substrate; Born-Infeld saturation; Ramanujan graphs; Kesten-McKay measure; valence growth; cascade exponent; spectral admissibility; Cheeger inequality},
  hidelinks
}
...
\begin{document}
  ...
  \subsection{Ramanujan--LPS graphs}
    ...
    \end{equation}
    The lower inequality is the content of the Alon--Boppana theorem~\cite{AlonBoppana}, an
    asymptotic near-optimality statement holding for regular graphs in general, not specific
    to the LPS family.
    The upper inequality is the Ramanujan property itself: it is a separate fact, established
    for the LPS family via Deligne's theorem on the Ramanujan--Petersson conjecture,
    following~\cite{LPS}, not a consequence of Alon--Boppana.
    ...
    $\equiv 0 \pmod q$, so $\alpha \equiv \pm\alpha' \pmod q$ forces $\alpha=\alpha'$.
    Hence $q > 2\sqrt p$ is a sufficient injectivity condition, guaranteeing $p+1$ pairwise
    distinct generators after reduction.
    Since $\mathrm{def{:}vv\_sequence}$'s own requirement $p(n) \to \infty$ forces this
    condition to be checked at every rank, a variable-valence sequence compatible with
  ...
  \subsection{Interpretation: expansion and the direction of the missing argument}
    ...
    It is stated here as an explicit conditional result -- a genuine implication, valid
    whenever its named premises hold, but with those premises left undissolved rather than
    derived -- not as a vague qualitative impression and not as ``not a theorem'' in the
    weaker sense of merely being unproved.

    \paragraph{The one unconditional ingredient: a weighted-union spectral estimate.}
    Let $E_n$ be the explored-configuration host of item~1 of
    Section~\ref{ssec:repair}, built as the multiplicity-weighted running union
    $M_n := \biguplus_{k \leq n} S_{p(k),q}$ of successive LPS generating sets (item~2 of
    Section~\ref{ssec:repair}).
    Writing $D_n := \sum_{k\leq n}(p(k)+1)$ for the multiplicity-counted degree and using that
    each individual shell satisfies the Ramanujan bound $\|T_\rho(S_{p(k),q})\| \leq
    2\sqrt{p(k)}$ on every nontrivial representation $\rho$ (Section~\ref{ssec:lps}), the
    triangle inequality gives, unconditionally and for every $n$,
    \begin{equation}
      \label{eq:tension-spectral}
      R_n \;:=\; \frac{2\sum_{k\leq n}\sqrt{p(k)}}{D_n} \;\geq\; \frac{\|A(M_n)|_\rho\|}{D_n}
      \quad\text{for every nontrivial } \rho,
    \end{equation}
    and for $p(k)\sim k^\beta$, $R_n = O(n^{-\beta/2}) \to 0$.
    This is the only ingredient in this subsection that is proved rather than assumed; it is
    a statement about the weighted multigraph $M_n$, not the simple union $E_n$, and
    transferring it to $E_n$ needs a separate bound on cross-shell generator overlap
    (Section~\ref{ssec:repair}, item~2), not supplied here.
    Granting that transfer, a vertex-isoperimetric inequality converts $R_n \to 0$ into a
    \emph{lower} bound $h_n$ on boundary size that grows toward its near-maximal value
    relative to the degree $d(n)=p(n)+1$; write this as $|\partial^+_V E_n| \geq h_n|E_n|$
    (outer vertex boundary) and, by degree-regularity, $|\partial^-_V E_n| \geq
    h_n|E_n|/d(n)$ (inner vertex boundary), valid only while $|E_n| \leq |G|/2$, the domain on
    which vertex-isoperimetric inequalities apply at all.

    \paragraph{The conditional bound.}
    Suppose, in addition to the transfer above and items 3--5 of Section~\ref{ssec:repair}
    (an edgewise flux-allocation law; the activation threshold $\epsilon_0 = \kappa c_\chi$
    with $\kappa$ dimensionless and $0 < \kappa \leq 1$, so that $b(n) :=
    \min(p(n)+1,\lfloor 1/\kappa\rfloor) \geq 1$; and an activation-utilisation and
    target-congestion mechanism), that a fixed fraction $\eta \in (0,1]$ of each
    inner-boundary vertex's activation budget $b(n)$ is realised, and that each exterior
    target vertex absorbs at most $C_n$ realised activations.
    Then, while $|E_n| \leq |G|/2$,
    \begin{equation}
      \label{eq:tension-bound}
      \Delta|E_n| \;\geq\; \frac{\eta\, b(n)}{C_n}\, \frac{h_n}{d(n)}\, |E_n|.
    \end{equation}
    Unlike $R_n \to 0$ above, none of $\eta$, $C_n$, or the cross-shell overlap transfer is
    derived; in particular, $b(n)\geq 1$ once $p(n)+1\geq 1/\kappa$ secures only that one
    factor, not $\eta$, $C_n$, or $h_n/d(n)$, which depend on separate, independent
    hypotheses.
    \emph{If} $\eta$, $b(n)$, $h_n/d(n)$, and $1/C_n$ are all bounded below by positive
    constants, \eqref{eq:tension-bound} is a geometric recursion for as long as $|E_n| \leq
    |G|/2$ continues to hold -- reaching that volume threshold on a depth scale logarithmic
    in $|G|$, not the polynomial growth $p(n) \sim n^\beta$ a structural exponent bound
    presupposes, but saying nothing about what happens once the threshold is reached, where
    the vertex-isoperimetric inequality itself no longer applies.

    This is a \textbf{conditional result on undelivered premises}, not merely an unproved
    claim: given the named hypotheses (the overlap transfer, $\eta$ bounded below, $C_n$
    bounded above), the geometric-growth conclusion follows validly up to the stated volume
    threshold; what is missing is a derivation, not merely a proof, of those hypotheses, and
    the conclusion is understood to fail if any of them fails to hold (e.g.\ $\eta \to 0$ or
    $C_n \to \infty$ fast enough).
    It identifies the exact burden of proof for a future repair: a mechanism supplying
    quantitative bounds on the overlap transfer, $\eta$, and $C_n$ -- not merely positing
    their existence -- that keeps~\eqref{eq:tension-bound}'s right-hand side sub-polynomial
    up to $|G|/2$, rather than a construction that simply assumes slow growth to recover the

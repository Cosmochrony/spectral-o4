...
\begin{document}
  ...
  \subsection{Ramanujan--LPS graphs}
    ...
    This condition is stated here for completeness; it does not by itself affect any of the
    findings of Section~\ref{sec:subdiffusion}, which concern the front-size argument rather
    than the choice between the two realisations.
  ...
  \subsection{Interpretation: expansion and the direction of the missing argument}
    ...
    though it does not settle the question, and is stated here as a precise conditional
    inequality rather than a general impression, so that its hypotheses are auditable.

    Write $E_n$ for the explored-configuration host of item~1 of Section~\ref{ssec:repair},
    $\partial^-_V E_n$ for its inner vertex boundary (vertices of $E_n$ adjacent to the
    complement) and $\partial^+_V E_n$ for its outer vertex boundary, $d(n)=p(n)+1$ for the
    degree, and $h_n$ for the vertex-isoperimetric constant supplied by the host's spectral
    gap (Section~\ref{ssec:repair}, item~1).
    Vertex-isoperimetric inequalities give only the \emph{lower} bound
    $|\partial^+_V E_n| \geq h_n |E_n|$ on available boundary, and $|\partial^-_V E_n| \geq
    h_n|E_n|/d(n)$ by degree-regularity.
    Suppose, in addition to items 3--5 of Section~\ref{ssec:repair} (the flux-allocation law,
    the threshold $\epsilon_0=\kappa c_\chi$, and the activation-utilisation/congestion
    mechanism), that a fixed fraction $\eta \in (0,1]$ of each inner-boundary vertex's
    activation budget $b(n) := \min(p(n)+1,\lfloor 1/\kappa\rfloor)$ is realised, and that
    each exterior target vertex absorbs at most $C_n$ realised activations.
    Then the number of newly explored vertices satisfies
    \begin{equation}
      \label{eq:tension-bound}
      \Delta|E_n| \;\geq\; \frac{\eta\, b(n)}{C_n}\, \frac{h_n}{d(n)}\, |E_n|.
    \end{equation}
    If $\eta$, $b(n)$, $h_n/d(n)$, and $1/C_n$ are all bounded below by positive constants
    over the window of Section~\ref{ssec:repair}, item~1 -- which holds, for instance, once
    $p(n)+1$ exceeds $1/\kappa$, under the multiplicity-weighted running-union spectral
    estimate of Section~\ref{ssec:repair} -- then~\eqref{eq:tension-bound} is a geometric
    recursion, giving finite-group saturation on a depth scale logarithmic in the host's size,
    not the polynomial growth $p(n) \sim n^\beta$ that a structural exponent bound
    presupposes.

    This is explicitly \textbf{not} a theorem: $\eta$ (activation utilisation) and $C_n$
    (target congestion) are named quantities that neither this note nor the corpus derives,
    and~\eqref{eq:tension-bound} is vacuous if either is left unbounded (e.g.\ $\eta \to 0$ or
    $C_n \to \infty$ fast enough).
    It identifies the exact burden of proof for a future repair: a mechanism supplying a
    quantitative bound on $\eta$ and $C_n$ -- not merely their existence -- that keeps the
    right-hand side of~\eqref{eq:tension-bound} sub-polynomial, rather than a construction
    that simply assumes slow growth to recover the phenomenological target.

...
\begin{document}
  ...
  \subsection{Ramanujan--LPS graphs}
    ...
    family of $(p+1)$-regular graphs of growing size, the largest nontrivial adjacency
    eigenvalue (in absolute value) cannot stay asymptotically smaller than $2\sqrt p$, so the
    Ramanujan property here is asymptotically optimal, not merely one admissible bound among
    many; it is not itself a source of either side of~\eqref{eq:ramanujan-laplacian}.
  ...
  \subsection{Interpretation: expansion and the direction of the missing argument}
    ...
    is a geometric recursion for as long as $|E_n|\leq|\mathcal G(n)|/2$ continues to hold --
    not the polynomial growth $p(n)\sim n^\beta$ a structural exponent bound presupposes.
    This says only that growth is geometric \emph{while} the volume hypothesis holds, not that
    the threshold $|\mathcal G(n)|/2$ is actually reached in any particular number of steps:
    since $|\mathcal G(n)|$ may itself vary with $n$ (through $q(n)$), concluding a specific
    depth scale (e.g.\ logarithmic in $|\mathcal G(n)|$) would need a further, separate
    hypothesis controlling how $|\mathcal G(n)|$ grows relative to the geometric rate, which
    is not stated here.
